...
add_action( 'wp_ajax_TotalSoftGallery_Video_Post', 'TotalSoftGallery_Video_Post_Callback' );
add_action( 'wp_ajax_nopriv_TotalSoftGallery_Video_Post', 'TotalSoftGallery_Video_Post_Callback' );

function TotalSoftGallery_Video_Post_Callback() {
	$val = sanitize_text_field( $_POST['video'] );
	$data = [];
	    $url_video = explode('/', $val);
	   
	

	    		if ( $url_video[2] == 'www.youtube.com' ) {
	    			 $id = $url_video[4];
                        ...
                        if ($httpCode != 404) {
                            /* Handle 404 here. */
                             $data = "https://img.youtube.com/vi/" . $id . "/maxresdefault.jpg";
                        } else {
                             $data = "https://img.youtube.com/vi/" . $id . "/mqdefault.jpg";
                        }
                        curl_close($handle);
                    		  ...
                    		  $data = $val;
	                        if ($url_video[0] == 'http:') {
	                            $url = str_replace("http", "https", $val);
	                            $handle = curl_init($url);
	                            curl_setopt($handle, CURLOPT_RETURNTRANSFER, TRUE);
	                            /* Get the HTML or whatever is linked in $url. */
	                            $response = curl_exec($handle);
...
add_action( 'wp_ajax_TotalSoftGallery_Page_Post', 'TotalSoftGallery_Page_Post_Callback' );
add_action( 'wp_ajax_nopriv_TotalSoftGallery_Page_Post', 'TotalSoftGallery_Page_Post_Callback' );

function TotalSoftGallery_Page_Post_Callback() {
	$val = sanitize_text_field( $_POST['page'] );
	$data = '';
	$url_pieces=[];
		$url_pieces = explode('/', $val);
	    
	

	    		if ($url_pieces[2] == 'img.youtube.com') {
	    			$id = $url_pieces[4];
                        $url = "https://img.youtube.com/vi/" . $id . "/maxresdefault.jpg";
                        $handle = curl_init($url);
                        curl_setopt($handle, CURLOPT_RETURNTRANSFER, TRUE);
