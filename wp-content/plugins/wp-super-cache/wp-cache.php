...
if (
	!isset($cache_enabled, $super_cache_enabled, $wp_cache_mod_rewrite, $cache_path) &&
...
?>
			<div class="notice notice-error">
				<h4><?php _e('CRON System Disabled', 'wp-super-cache'); ?></h4>
			...
			?><div class="notice notice-warning">
					<h4><?php printf(__('Warning! Your hostname "%s" resolves to %s', 'wp-super-cache'), $hostname, $ip); ?></h4>
			...
			if (is_array($cron)) {
				...
				?><div class="notice notice-error">
						<h4>Warning! wp-cron.php not found!</h4>
	...
	if (1 == ini_get('zlib.output_compression') || "on" == strtolower(ini_get('zlib.output_compression'))) {
		?><div class="notice notice-warning">
			<h4><?php _e('Zlib Output Compression Enabled!', 'wp-super-cache'); ?></h4>
			...
			if (
				...
				?><div class="notice notice-warning">
			<h4><?php _e('Mod rewrite may not be installed!', 'wp-super-cache'); ?></h4>
			<p><?php _e('It appears that mod_rewrite is not installed. Sometimes this check isn&#8217;t 100% reliable, especially if you are not using Apache. Please verify that the mod_rewrite module is loaded. It is required for serving Super Cache static files in expert mode. You will still be able to simple mode.', 'wp-super-cache'); ?></p>
		</div><?php
			}
			if (!is_writeable_ACLSafe($wp_cache_config_file)) {
				...
				?><div class="notice notice-error">
			<h4><?php _e('Read Only Mode. Configuration cannot be changed.', 'wp-super-cache'); ?></h4>
			...
			if ($cache_enabled && (!isset($wp_cache_shutdown_gc) || $wp_cache_shutdown_gc == 0) && function_exists('get_gc_flag')) {
				...
				?><div class="notice notice-warning">
					<h4><?php _e('Warning! Garbage collection is not scheduled!', 'wp-super-cache'); ?></h4>
			...
			?><div class="notice notice-warning">
				<h4><?php printf(__('Warning! %s is writeable!', 'wp-super-cache'), WP_CONTENT_DIR); ?></h4>
		...
		<?php
			...
			if (!$is_nginx && function_exists("is_main_site") && true == is_main_site()) {
				...
				if ($cache_enabled && $wp_cache_mod_rewrite && !$wp_cache_mobile_enabled && strpos($scrules, addcslashes(str_replace(', ', '|', $wp_cache_mobile_browsers), ' '))) {
					...
					$wp_cache_mod_rewrite && $cache_enabled && $wp_cache_mobile_enabled && $scrules != '' && (
		...
		?>
			<div class="notice notice-warning">
				<h4><?php _e('Rewrite rules must be updated', 'wp-super-cache'); ?></h4>
				...
				<ol>
					<li> <?php _e('Set the plugin to simple mode and enable mobile support.', 'wp-super-cache'); ?></li>
					...
					<li> <?php printf(__('Add the rules yourself. Edit %s.htaccess and find the block of code enclosed by the lines <code># BEGIN WPSuperCache</code> and <code># END WPSuperCache</code>. There are two sections that look very similar. Just below the line <code>%%{HTTP:Cookie} !^.*(comment_author_|%s|wp-postpass_).*$</code> add these lines: (do it twice, once for each section)', 'wp-super-cache'), $home_path, wpsc_get_logged_in_cookie()); ?></p>
						<div style='padding: 2px; margin: 2px; border: 1px solid #333; width:400px; overflow: scroll'>
							<pre><?php echo "RewriteCond %{HTTP_user_agent} !^.*(" . addcslashes(str_replace(', ', '|', $wp_cache_mobile_browsers), ' ') . ").*\nRewriteCond %{HTTP_user_agent} !^(" . addcslashes(str_replace(', ', '|', $wp_cache_mobile_prefixes), ' ') . ").*"; ?></pre>
						</div>
					</li>
				</ol>
			</div><?php
			...
			if (!$is_nginx && $wp_cache_mod_rewrite && $super_cache_enabled && function_exists('apache_get_modules')) {
				...
				if (isset($missing_mods) && is_array($missing_mods)) {
					?><div class='notice notice-warning'>
				<h4><?php _e('Missing Apache Modules', 'wp-super-cache'); ?></h4>
				...
				<h4><?php _e('.htaccess file may need to be moved', 'wp-super-cache'); ?></h4>
				...
				<br />
																																																									...
																																																									function admin_bar_delete_page()
																																																									{
																																																										...
																																																										if ($path) {
																																																											...
																																																											if (
																																																												false === wp_cache_confirm_delete($path) ||
				...
				?>
						<table class="form-table">
							<tr valign="top">
								...
								<td>
									<fieldset>
										...
										<label><input type='checkbox' name='wp_cache_enabled' value='1' <?php if ($cache_enabled == true) {
							...
							<tr valign="top">
								...
								<td>
									<fieldset>
										<label><input type='radio' name='wp_cache_mod_rewrite' <?php if ($wp_cache_mod_rewrite == 0) echo "checked"; ?> value='0'> <?php _e('<acronym title="Use PHP to serve cached files">Simple</acronym>', 'wp-super-cache');
						...
						<?php
																																																												...
																																																												if ($cache_enabled) {
																																																													...
																																																													if (array_key_exists('action', $_POST) && 'test' === $_POST['action'] && $valid_nonce) {
																																																														...
																																																														if (
																																																															false == $cache_test_error && preg_match('/(Cached page generated by WP-Super-Cache on) ([0-9]*-[0-9]*-[0-9]* [0-9]*:[0-9]*:[0-9]*)/', $page[1]['body'], $matches1) &&
			...
			<fieldset class="options">
				<h4><?php _e('Directly Cached Files', 'wp-super-cache'); ?></h4><?php
							...
							<tr>
								...
								<th><?php _e('Delete cached file', 'wp-super-cache'); ?></th>
							</tr><?php
																																																																																																																																																																																						echo "$out</table>";
																																																																																																																																																																																					}
																																																																																																																																																																																				}
																																																									...
																																																									function wp_cache_time_update()
																																																									{
																																																										...
																																																										if (isset($_POST['action']) && $_POST['action'] == 'expirytime') {
																																																											...
																																																											if (isset($_POST['cache_schedule_type']) && $_POST['cache_schedule_type'] == 'interval' && isset($_POST['cache_time_interval'])) {
																																																												...
																																																												if (
																																																													!isset($_POST['cache_scheduled_time']) ||
...
&lt;!-- super cache --></pre></td></tr>";
																																																									...
																																																									function wp_cache_index_notice()
																																																									{
																																																										...
																																																										if (
																																																											strlen($cache_path) < strlen(ABSPATH)
																																																									...
																																																									function wp_cache_add_index_protection()
																																																									{
																																																										...
																																																										if (is_dir($cache_path) && false == is_file("$cache_path/index.html")) {
																																																											...
																																																											if (false == is_wp_error($page)) {
																																																												if (
																																																													false == get_site_option('wp_super_cache_index_detected')
